...
export default function MainContainer() {
  const livingDaysAlex = Math.round((new Date().getTime() - new Date(1997, 11, 14).getTime()) / 86400000)
  const livingDaysVitor = Math.round((new Date().getTime() - new Date(1993, 9, 13).getTime()) / 86400000)

  const alex: IUser = {
    name: "alexercolin",
    fullName: "Alex Ercolin Oliveira",
    description:
      `Olá usuários da internet ✌️ Meu nome é Alex. Sou formado em Engenharia de Produção pela FACENS. Tenho dias de idade ${livingDaysAlex} (a gente fez a continha com js). Iniciei minha trajetória profissional  ̶f̶a̶z̶e̶n̶d̶o̶ ̶m̶u̶i̶t̶o̶s̶ ̶c̶a̶f̶é̶s̶ no setor de vendas e passei um bom tempo em engenheria de processos, mas sempre com um pé em Tecnologia. Aí o gerente ficou louco e decidiu começar do zero e se tornar um desenvolvedor. Atualmente atuo como Full-Stack Developer (nome chique né?) trabalhando com a minha linguagem de programação favorita: JavaScript. `,
